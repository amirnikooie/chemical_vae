...
def sampled_rnn(step_function, inputs, initial_states, units, random_seed,
   ...
   """
    ...
    axes = [1, 0] + list(range(2, ndim)) # Same as original code.
    inputs = tf.transpose(inputs, (axes))

    if go_backwards: # From original code this is for when unroll is None, as explained in line 73 above!!
    ...
    output_dim = int(initial_states[0].get_shape()[-1]) # This line and next line is unique to this code and so a difference.
    random_cutoff_prob = tf.random.uniform(
        (num_samples,), minval=0., maxval=1.)

    # Ignore constants for the first run
    outputs, _ = step_function(inputs[0], {'initial_states': initial_states, # Different input arguments to this function compared to the original code. so this a difference.
    ...
    def _step(time, output_ta_t, *states): # Compared to the original code, this is the one with mask is None! As explained in line 75 above!!  The difference
                                           # is only 1 line in the _step function including mask_t and tiling and dealing with tile_mask_t, etc.
        """RNN step function.
        ...
        random_cutoff_prob = tf.random.uniform( # This random_cutoff_prob is unique to this code and so this is a difference.
            (num_samples,), minval=0, maxval=1)

        output, new_states = step_function(current_input, # The inputs to this function is unique for this code and thus a difference. But it exists already.
                                           {'initial_states': states,
                                            'random_cutoff_prob': random_cutoff_prob,
                                            'rec_dp_mask': rec_dp_constants})
        # returned output is ( raw/sampled, batch, output_dim)
        axes = [1, 0] + list(range(2, K.ndim(output))) # This line and the next line (the one before for) are unique to this code not existing int he origignal. This is a difference.
    ...
    final_outputs = control_flow_ops.while_loop(
        ...
        parallel_iterations=1, # it is 32 in the original code. So, this is a difference
        swap_memory=True)
